          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {editingServices.map((service, index) => (
              ...
              >
                ...
                <p className="font-nunito font-bold text-text-gray mb-6 leading-relaxed">
                  {service.description}
                </p>
                <ul className="space-y-2">
                  {service.features.map((feature, idx) => (
                    <li key={idx} className="flex items-center space-x-2">
                      <div className="w-2 h-2 bg-text-gray rounded-full"></div>
                      <span className="font-nunito font-bold text-text-gray">{feature}</span>
                    </li>
                  ))}
                </ul>
              </div>
